#ifndef _smirnov224_13_91_approx_HPP_
#define _smirnov224_13_91_approx_HPP_

// This is an automatically generated file from gen.py.
#include "common.hpp"

namespace smirnov224_13_91_approx {
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataS1[i + j * strideS1] + Scalar(x) * dataS2[i + j * strideS2] + dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(x) * dataS1[i + j * strideS1] + Scalar(-(x * x)) * dataS2[i + j * strideS2] + dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(x) * dataS1[i + j * strideS1] -dataS2[i + j * strideS2] + Scalar(x) * dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x * x)) * dataS1[i + j * strideS1] + dataS2[i + j * strideS2] + Scalar(x) * dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(x * x) * dataT1[i + j * strideT1] + Scalar(-(x)) * dataT2[i + j * strideT2] + Scalar(x) * dataT3[i + j * strideT3] -dataT4[i + j * strideT4] -dataT5[i + j * strideT5];
        }
    }
}

template <typename Scalar>
void T_Add4(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& T3, Matrix<Scalar>& T4, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideT3 = T3.stride();
    const int strideT4 = T4.stride();
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataT1[i + j * strideT1] + Scalar(-(x)) * dataT2[i + j * strideT2] + dataT3[i + j * strideT3] + dataT4[i + j * strideT4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(x * x) * dataT1[i + j * strideT1] + Scalar(x) * dataT2[i + j * strideT2] + dataT3[i + j * strideT3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar((x + x * x)) * dataT1[i + j * strideT1] + dataT2[i + j * strideT2];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataT1[i + j * strideT1] -dataT2[i + j * strideT2] + Scalar(x) * dataT3[i + j * strideT3] + dataT4[i + j * strideT4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(-(1.0 / (x * x))) * dataM1[i + j * strideM1] + Scalar(-(1.0 / (x * x))) * dataM2[i + j * strideM2] + Scalar(1.0 / (x * x)) * dataM3[i + j * strideM3] + Scalar(1.0 / (x * x)) * dataM4[i + j * strideM4] + Scalar(-(1.0 / (x * x))) * dataM5[i + j * strideM5] + Scalar(1.0 / (x * x)) * dataM6[i + j * strideM6] + Scalar(-(1.0 / (x * x))) * dataM7[i + j * strideM7] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(-(1.0 / (x * x))) * dataM1[i + j * strideM1] + Scalar(-(1.0 / (x * x))) * dataM2[i + j * strideM2] + Scalar(1.0 / (x * x)) * dataM3[i + j * strideM3] + Scalar(1.0 / (x * x)) * dataM4[i + j * strideM4] + Scalar(-(1.0 / (x * x))) * dataM5[i + j * strideM5] + Scalar(1.0 / (x * x)) * dataM6[i + j * strideM6] + Scalar(-(1.0 / (x * x))) * dataM7[i + j * strideM7];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(-(1.0 / (x))) * dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(-(1.0 / (x))) * dataM3[i + j * strideM3] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(-(1.0 / (x))) * dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(-(1.0 / (x))) * dataM3[i + j * strideM3];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] + Scalar(1.0 / (x * x)) * dataM2[i + j * strideM2] + Scalar(-(1.0 / (x * x))) * dataM3[i + j * strideM3] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] + Scalar(1.0 / (x * x)) * dataM2[i + j * strideM2] + Scalar(-(1.0 / (x * x))) * dataM3[i + j * strideM3];
            }
        }
    }
}
...
void FastMatmulRecursive(LockAndCounter& locker, MemoryManager<Scalar>& mem_mngr, Matrix<Scalar>& A, Matrix<Scalar>& B, Matrix<Scalar>& C, int total_steps, int steps_left, int start_index, double x, int num_threads, Scalar beta) {
    ...
    bool sequential1 = should_launch_task(13, total_steps, steps_left, start_index, 1, num_threads);
    bool sequential2 = should_launch_task(13, total_steps, steps_left, start_index, 2, num_threads);
    bool sequential3 = should_launch_task(13, total_steps, steps_left, start_index, 3, num_threads);
    bool sequential4 = should_launch_task(13, total_steps, steps_left, start_index, 4, num_threads);
    bool sequential5 = should_launch_task(13, total_steps, steps_left, start_index, 5, num_threads);
    bool sequential6 = should_launch_task(13, total_steps, steps_left, start_index, 6, num_threads);
    bool sequential7 = should_launch_task(13, total_steps, steps_left, start_index, 7, num_threads);
    bool sequential8 = should_launch_task(13, total_steps, steps_left, start_index, 8, num_threads);
    bool sequential9 = should_launch_task(13, total_steps, steps_left, start_index, 9, num_threads);
    bool sequential10 = should_launch_task(13, total_steps, steps_left, start_index, 10, num_threads);
    bool sequential11 = should_launch_task(13, total_steps, steps_left, start_index, 11, num_threads);
    bool sequential12 = should_launch_task(13, total_steps, steps_left, start_index, 12, num_threads);
    bool sequential13 = should_launch_task(13, total_steps, steps_left, start_index, 13, num_threads);
#else
    ...
    // M1 = (-1.0 * A11 + 1.0 * A22) * (-(x) * B21 + 1.0 * B22 + 1.0 * B24)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add1(B21, B22, B24, T1, x, sequential1);
    FastMatmulRecursive(locker, mem_mngr, S1, T1, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S1.deallocate();
    T1.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 1, num_threads)) {
# pragma omp taskwait
...
# endif
    ...
    // M2 = (-(x) * A12 + 1.0 * A22) * (x * B21 + -1.0 * B22)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
# pragma omp task if(sequential2) shared(mem_mngr, locker) untied default(shared)
    ...
    FastMatmulRecursive(locker, mem_mngr, S2, T2, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S2.deallocate();
    T2.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 2, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M3 = (1.0 * A11) * (x * x * B11 + -(x) * B12 + x * B21 + -1.0 * B22 + -1.0 * B24)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add3(B11, B12, B21, B22, B24, T3, x, sequential3);
    M3.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A11, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T3.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 3, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M4 = (-1.0 * A11 + x * A12) * (-(x) * B14 + -(x) * B21 + 1.0 * B22 + 1.0 * B24)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add4(B14, B21, B22, B24, T4, x, sequential4);
    FastMatmulRecursive(locker, mem_mngr, S4, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S4.deallocate();
    T4.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 4, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
# pragma omp task if(sequential5) shared(mem_mngr, locker) untied default(shared)
    ...
    M5.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A22, B24, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    }
    if (should_task_wait(13, total_steps, steps_left, start_index, 5, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M6 = (-(x) * A11 + x * A12 + 1.0 * A21) * (1.0 * B12)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add6(A11, A12, A21, S6, x, sequential6);
    M6.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, S6, B12, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S6.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 6, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M7 = (-(x) * A12 + 1.0 * A21) * (x * x * B13 + x * B23 + 1.0 * B24)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add7(A12, A21, S7, x, sequential7);
#ifdef _PARALLEL_
    Matrix<Scalar> T7(mem_mngr.GetMem(start_index, 7, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T7(B11.m(), B11.n());
#endif
    T_Add7(B13, B23, B24, T7, x, sequential7);
    FastMatmulRecursive(locker, mem_mngr, S7, T7, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S7.deallocate();
    T7.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 7, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M8 = (x * A11 + -(x * x) * A12 + 1.0 * A21) * ((x + x * x) * B13 + 1.0 * B14)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add8(B13, B14, T8, x, sequential8);
    FastMatmulRecursive(locker, mem_mngr, S8, T8, M8, total_steps, steps_left - 1, (start_index + 8 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S8.deallocate();
    T8.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 8, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M9 = (x * A12 + -1.0 * A21 + x * A22) * (x * B23 + 1.0 * B24)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add9(B23, B24, T9, x, sequential9);
    FastMatmulRecursive(locker, mem_mngr, S9, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S9.deallocate();
    T9.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 9, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M10 = (-(x * x) * A12 + 1.0 * A21 + x * A22) * (x * x * B11 + 1.0 * B22)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add10(A12, A21, A22, S10, x, sequential10);
#ifdef _PARALLEL_
    Matrix<Scalar> T10(mem_mngr.GetMem(start_index, 10, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T10(B11.m(), B11.n());
#endif
    T_Add10(B11, B22, T10, x, sequential10);
    FastMatmulRecursive(locker, mem_mngr, S10, T10, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S10.deallocate();
    T10.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 10, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M11 = (x * A12 + 1.0 * A21) * (1.0 * B12 + 1.0 * B22)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
# pragma omp task if(sequential11) shared(mem_mngr, locker) untied default(shared)
...
#endif
    S_Add11(A12, A21, S11, x, sequential11);
#ifdef _PARALLEL_
    ...
    FastMatmulRecursive(locker, mem_mngr, S11, T11, M11, total_steps, steps_left - 1, (start_index + 11 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S11.deallocate();
    T11.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 11, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M12 = (1.0 * A21) * (-(x) * B13 + -1.0 * B14 + x * B23 + 1.0 * B24)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add12(B13, B14, B23, B24, T12, x, sequential12);
    M12.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A21, T12, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T12.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(13, total_steps, steps_left, start_index, 12, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
# pragma omp task if(sequential13) shared(mem_mngr, locker) untied default(shared)
    ...
    M13.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A21, B22, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 13, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    }
    if (should_task_wait(13, total_steps, steps_left, start_index, 13, num_threads)) {
# pragma omp taskwait
    }
#endif

    M_Add1(M1, M2, M3, M5, M6, M11, M13, C11, x, false, beta);
    M_Add2(M6, M11, M13, C12, x, false, beta);
    M_Add3(M1, M2, M4, M5, M7, M8, M12, C13, x, false, beta);
    M_Add4(M1, M2, M4, M5, C14, x, false, beta);
    M_Add5(M2, M10, M13, C21, x, false, beta);
    M_Add6(M1, M3, M5, M6, C22, x, false, beta);
    M_Add7(M5, M7, M9, C23, x, false, beta);
    M_Add8(M5, M8, C24, x, false, beta);
...
#endif
    DynamicPeeling(A, B, C, 2, 2, 4, beta);
}
...
#ifdef _PARALLEL_
    mem_mngr.Allocate(2, 2, 4, 13, num_steps, A.m(), A.n(), B.n());
#endif
    A.set_multiplier(alpha);
    int num_multiplies_per_step = 13;
...
#endif  // _smirnov224_13_91_approx_HPP_
