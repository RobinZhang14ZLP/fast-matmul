#ifndef _smirnov552_37_262_approx_HPP_
#define _smirnov552_37_262_approx_HPP_

// This is an automatically generated file from gen.py.
#include "common.hpp"

namespace smirnov552_37_262_approx {
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataS1[i + j * strideS1] + Scalar(x * x) * dataS2[i + j * strideS2] + Scalar(1.0 / (x)) * dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = dataS1[i + j * strideS1] + dataS2[i + j * strideS2] + Scalar(-(1.0 / (x))) * dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = -dataS1[i + j * strideS1] + Scalar(x * x) * dataS2[i + j * strideS2] + Scalar(1.0 / (x)) * dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = dataS1[i + j * strideS1] + dataS2[i + j * strideS2] + Scalar(x) * dataS3[i + j * strideS3] + Scalar(-(1.0 / (x))) * dataS4[i + j * strideS4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(x) * dataS1[i + j * strideS1] + Scalar(x * x) * dataS2[i + j * strideS2] -dataS3[i + j * strideS3] + Scalar(1.0 / (x)) * dataS4[i + j * strideS4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = dataS1[i + j * strideS1] + Scalar(-(1.0 / (x))) * dataS2[i + j * strideS2] + Scalar(x * x) * dataS3[i + j * strideS3] + dataS4[i + j * strideS4] -dataS5[i + j * strideS5];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataS1[i + j * strideS1] + Scalar(x * x) * dataS2[i + j * strideS2] + Scalar(x) * dataS3[i + j * strideS3] + dataS4[i + j * strideS4] + Scalar(1.0 / (x)) * dataS5[i + j * strideS5];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataS1[i + j * strideS1] + Scalar(x * x) * dataS2[i + j * strideS2] + Scalar(x) * dataS3[i + j * strideS3] + Scalar(1.0 / (x)) * dataS4[i + j * strideS4] + Scalar(-(1.0 / (x))) * dataS5[i + j * strideS5];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(1.0 / (x))) * dataS1[i + j * strideS1] + Scalar(x * x) * dataS2[i + j * strideS2] + Scalar(x) * dataS3[i + j * strideS3] + Scalar(-(1.0 / (x))) * dataS4[i + j * strideS4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataS1[i + j * strideS1] + dataS2[i + j * strideS2] + Scalar(1.0 / (x)) * dataS3[i + j * strideS3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = -dataS1[i + j * strideS1] + dataS2[i + j * strideS2] + dataS3[i + j * strideS3] + Scalar(-(1.0 / (x))) * dataS4[i + j * strideS4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = -dataS1[i + j * strideS1] + Scalar(-(x)) * dataS2[i + j * strideS2] + dataS3[i + j * strideS3] + Scalar(x * x) * dataS4[i + j * strideS4];
        }
    }
}
...
template <typename Scalar>
void S_Add31(Matrix<Scalar>& S1, Matrix<Scalar>& S2, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideS1 = S1.stride();
    const int strideS2 = S2.stride();
    const int strideC = C.stride();
    const Scalar *dataS1 = S1.data();
...
template <typename Scalar>
void S_Add34(Matrix<Scalar>& S1, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideS1 = S1.stride();
    const int strideC = C.stride();
    const Scalar *dataS1 = S1.data();
    Scalar *dataC = C.data();
...
template <typename Scalar>
void S_Add37(Matrix<Scalar>& S1, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideS1 = S1.stride();
    const int strideC = C.stride();
    const Scalar *dataS1 = S1.data();
    Scalar *dataC = C.data();
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = dataT1[i + j * strideT1] + Scalar(1.0 / (x)) * dataT2[i + j * strideT2] + Scalar(1.0 / (x)) * dataT3[i + j * strideT3] + Scalar(-(x)) * dataT4[i + j * strideT4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = dataT1[i + j * strideT1] + Scalar(1.0 / (x)) * dataT2[i + j * strideT2] + Scalar(-(x)) * dataT3[i + j * strideT3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x)) * dataT1[i + j * strideT1] + Scalar(-(x * x)) * dataT2[i + j * strideT2] + Scalar(x * x) * dataT3[i + j * strideT3] + dataT4[i + j * strideT4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(x) * dataT1[i + j * strideT1] + Scalar(-(x)) * dataT2[i + j * strideT2] + Scalar(1.0 / (x)) * dataT3[i + j * strideT3];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = dataT1[i + j * strideT1] + Scalar(1.0 / (x)) * dataT2[i + j * strideT2] + Scalar(1.0 / (x)) * dataT3[i + j * strideT3] + dataT4[i + j * strideT4];
        }
    }
}
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(-(x * x)) * dataT1[i + j * strideT1] + Scalar(1.0 / (x)) * dataT2[i + j * strideT2] + dataT3[i + j * strideT3] + Scalar(x) * dataT4[i + j * strideT4];
        }
    }
}
...
template <typename Scalar>
void T_Add26(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& T3, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideT3 = T3.stride();
    const int strideC = C.stride();
...
# pragma omp parallel for if(!sequential)
    ...
    for (int j = 0; j < C.n(); ++j) {
        for (int i = 0; i < C.m(); ++i) {
            dataC[i + j * strideC] = Scalar(x) * dataT1[i + j * strideT1] + Scalar(1.0 / (x)) * dataT2[i + j * strideT2] + Scalar(-(1.0 / (x))) * dataT3[i + j * strideT3];
        }
    }
}
...
template <typename Scalar>
void T_Add28(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideC = C.stride();
    const Scalar *dataT1 = T1.data();
...
template <typename Scalar>
void T_Add31(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideC = C.stride();
    const Scalar *dataT1 = T1.data();
...
template <typename Scalar>
void T_Add33(Matrix<Scalar>& T1, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideC = C.stride();
    const Scalar *dataT1 = T1.data();
    Scalar *dataC = C.data();
...
template <typename Scalar>
void T_Add34(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& T3, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideT3 = T3.stride();
    const int strideC = C.stride();
...
template <typename Scalar>
void T_Add35(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideC = C.stride();
    const Scalar *dataT1 = T1.data();
...
template <typename Scalar>
void T_Add36(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& T3, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideT3 = T3.stride();
    const int strideC = C.stride();
...
template <typename Scalar>
void T_Add37(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& T3, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideT3 = T3.stride();
    const int strideC = C.stride();
...
template <typename Scalar>
void M_Add1(Matrix<Scalar>& M1, Matrix<Scalar>& M2, Matrix<Scalar>& M3, Matrix<Scalar>& M4, Matrix<Scalar>& M5, Matrix<Scalar>& M6, Matrix<Scalar>& M7, Matrix<Scalar>& C, double x, bool sequential, Scalar beta) {
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] -dataM2[i + j * strideM2] + Scalar(-(1.0 / (x))) * dataM3[i + j * strideM3] + Scalar(-(1.0 / (x))) * dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5] + Scalar(1.0 / (x)) * dataM6[i + j * strideM6] + Scalar(x) * dataM7[i + j * strideM7] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] -dataM2[i + j * strideM2] + Scalar(-(1.0 / (x))) * dataM3[i + j * strideM3] + Scalar(-(1.0 / (x))) * dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5] + Scalar(1.0 / (x)) * dataM6[i + j * strideM6] + Scalar(x) * dataM7[i + j * strideM7];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(x) * dataM1[i + j * strideM1] + Scalar(x) * dataM2[i + j * strideM2] -dataM3[i + j * strideM3] + Scalar(x) * dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5] + Scalar(x * x) * dataM6[i + j * strideM6] + Scalar(x) * dataM7[i + j * strideM7] + Scalar(x) * dataM8[i + j * strideM8] + Scalar(x) * dataM9[i + j * strideM9] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(x) * dataM1[i + j * strideM1] + Scalar(x) * dataM2[i + j * strideM2] -dataM3[i + j * strideM3] + Scalar(x) * dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5] + Scalar(x * x) * dataM6[i + j * strideM6] + Scalar(x) * dataM7[i + j * strideM7] + Scalar(x) * dataM8[i + j * strideM8] + Scalar(x) * dataM9[i + j * strideM9];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(-(1.0 / (x))) * dataM3[i + j * strideM3] + Scalar(1.0 / (x)) * dataM4[i + j * strideM4] + Scalar(1.0 / (x)) * dataM5[i + j * strideM5] + Scalar(1.0 / (x)) * dataM6[i + j * strideM6] + Scalar(1.0 / (x)) * dataM7[i + j * strideM7] + Scalar(-(1.0 / (x))) * dataM8[i + j * strideM8] + Scalar(x) * dataM9[i + j * strideM9] + Scalar(x) * dataM10[i + j * strideM10] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(-(1.0 / (x))) * dataM3[i + j * strideM3] + Scalar(1.0 / (x)) * dataM4[i + j * strideM4] + Scalar(1.0 / (x)) * dataM5[i + j * strideM5] + Scalar(1.0 / (x)) * dataM6[i + j * strideM6] + Scalar(1.0 / (x)) * dataM7[i + j * strideM7] + Scalar(-(1.0 / (x))) * dataM8[i + j * strideM8] + Scalar(x) * dataM9[i + j * strideM9] + Scalar(x) * dataM10[i + j * strideM10];
            }
        }
    }
}
...
template <typename Scalar>
void M_Add4(Matrix<Scalar>& M1, Matrix<Scalar>& M2, Matrix<Scalar>& M3, Matrix<Scalar>& M4, Matrix<Scalar>& M5, Matrix<Scalar>& M6, Matrix<Scalar>& M7, Matrix<Scalar>& M8, Matrix<Scalar>& M9, Matrix<Scalar>& M10, Matrix<Scalar>& M11, Matrix<Scalar>& C, double x, bool sequential, Scalar beta) {
    const int strideM1 = M1.stride();
    const int strideM2 = M2.stride();
    const int strideM3 = M3.stride();
    const int strideM4 = M4.stride();
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = -dataM1[i + j * strideM1] -dataM2[i + j * strideM2] + Scalar(1.0 / (x)) * dataM3[i + j * strideM3] + Scalar(x) * dataM4[i + j * strideM4] -dataM5[i + j * strideM5] + dataM6[i + j * strideM6] + Scalar(1.0 / (x)) * dataM7[i + j * strideM7] + dataM8[i + j * strideM8] + Scalar(1.0 / (x)) * dataM9[i + j * strideM9] + Scalar(-(x)) * dataM10[i + j * strideM10] + Scalar(-(x)) * dataM11[i + j * strideM11] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = -dataM1[i + j * strideM1] -dataM2[i + j * strideM2] + Scalar(1.0 / (x)) * dataM3[i + j * strideM3] + Scalar(x) * dataM4[i + j * strideM4] -dataM5[i + j * strideM5] + dataM6[i + j * strideM6] + Scalar(1.0 / (x)) * dataM7[i + j * strideM7] + dataM8[i + j * strideM8] + Scalar(1.0 / (x)) * dataM9[i + j * strideM9] + Scalar(-(x)) * dataM10[i + j * strideM10] + Scalar(-(x)) * dataM11[i + j * strideM11];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(1.0 / (x)) * dataM3[i + j * strideM3] + Scalar(1.0 / (x)) * dataM4[i + j * strideM4] + Scalar(1.0 / (x)) * dataM5[i + j * strideM5] + Scalar(1.0 / (x)) * dataM6[i + j * strideM6] + Scalar(-(1.0 / (x))) * dataM7[i + j * strideM7] + Scalar(1.0 / (x)) * dataM8[i + j * strideM8] + dataM9[i + j * strideM9] -dataM10[i + j * strideM10] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(1.0 / (x)) * dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(1.0 / (x)) * dataM3[i + j * strideM3] + Scalar(1.0 / (x)) * dataM4[i + j * strideM4] + Scalar(1.0 / (x)) * dataM5[i + j * strideM5] + Scalar(1.0 / (x)) * dataM6[i + j * strideM6] + Scalar(-(1.0 / (x))) * dataM7[i + j * strideM7] + Scalar(1.0 / (x)) * dataM8[i + j * strideM8] + dataM9[i + j * strideM9] -dataM10[i + j * strideM10];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(1.0 / (x)) * dataM3[i + j * strideM3] + Scalar(1.0 / (x)) * dataM4[i + j * strideM4] + Scalar(1.0 / (x)) * dataM5[i + j * strideM5] -dataM6[i + j * strideM6] -dataM7[i + j * strideM7] + dataM8[i + j * strideM8] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = dataM1[i + j * strideM1] + Scalar(1.0 / (x)) * dataM2[i + j * strideM2] + Scalar(1.0 / (x)) * dataM3[i + j * strideM3] + Scalar(1.0 / (x)) * dataM4[i + j * strideM4] + Scalar(1.0 / (x)) * dataM5[i + j * strideM5] -dataM6[i + j * strideM6] -dataM7[i + j * strideM7] + dataM8[i + j * strideM8];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = dataM1[i + j * strideM1] + Scalar(x) * dataM2[i + j * strideM2] + dataM3[i + j * strideM3] + dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = dataM1[i + j * strideM1] + Scalar(x) * dataM2[i + j * strideM2] + dataM3[i + j * strideM3] + dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5];
            }
        }
    }
}
...
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(-(x)) * dataM1[i + j * strideM1] + Scalar(-(x)) * dataM2[i + j * strideM2] + Scalar(x) * dataM3[i + j * strideM3] + Scalar(x * x) * dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5] + Scalar(x) * dataM6[i + j * strideM6] + Scalar(x) * dataM7[i + j * strideM7] + Scalar(x) * dataM8[i + j * strideM8] + Scalar(x) * dataM9[i + j * strideM9] + Scalar(x) * dataM10[i + j * strideM10] + Scalar(x) * dataM11[i + j * strideM11] + beta * dataC[i + j * strideC];
            }
        }
    } else {
#ifdef _PARALLEL_
# pragma omp parallel for if(!sequential)
        ...
        for (int j = 0; j < C.n(); ++j) {
            for (int i = 0; i < C.m(); ++i) {
                dataC[i + j * strideC] = Scalar(-(x)) * dataM1[i + j * strideM1] + Scalar(-(x)) * dataM2[i + j * strideM2] + Scalar(x) * dataM3[i + j * strideM3] + Scalar(x * x) * dataM4[i + j * strideM4] + Scalar(x) * dataM5[i + j * strideM5] + Scalar(x) * dataM6[i + j * strideM6] + Scalar(x) * dataM7[i + j * strideM7] + Scalar(x) * dataM8[i + j * strideM8] + Scalar(x) * dataM9[i + j * strideM9] + Scalar(x) * dataM10[i + j * strideM10] + Scalar(x) * dataM11[i + j * strideM11];
            }
        }
    }
}
...
template <typename Scalar>
void M_Add10(Matrix<Scalar>& M1, Matrix<Scalar>& M2, Matrix<Scalar>& M3, Matrix<Scalar>& M4, Matrix<Scalar>& M5, Matrix<Scalar>& M6, Matrix<Scalar>& M7, Matrix<Scalar>& M8, Matrix<Scalar>& C, double x, bool sequential, Scalar beta) {
    const int strideM1 = M1.stride();
    const int strideM2 = M2.stride();
    const int strideM3 = M3.stride();
    const int strideM4 = M4.stride();
...
void FastMatmulRecursive(LockAndCounter& locker, MemoryManager<Scalar>& mem_mngr, Matrix<Scalar>& A, Matrix<Scalar>& B, Matrix<Scalar>& C, int total_steps, int steps_left, int start_index, double x, int num_threads, Scalar beta) {
    ...
    bool sequential1 = should_launch_task(37, total_steps, steps_left, start_index, 1, num_threads);
    bool sequential2 = should_launch_task(37, total_steps, steps_left, start_index, 2, num_threads);
    bool sequential3 = should_launch_task(37, total_steps, steps_left, start_index, 3, num_threads);
    bool sequential4 = should_launch_task(37, total_steps, steps_left, start_index, 4, num_threads);
    bool sequential5 = should_launch_task(37, total_steps, steps_left, start_index, 5, num_threads);
    bool sequential6 = should_launch_task(37, total_steps, steps_left, start_index, 6, num_threads);
    bool sequential7 = should_launch_task(37, total_steps, steps_left, start_index, 7, num_threads);
    bool sequential8 = should_launch_task(37, total_steps, steps_left, start_index, 8, num_threads);
    bool sequential9 = should_launch_task(37, total_steps, steps_left, start_index, 9, num_threads);
    bool sequential10 = should_launch_task(37, total_steps, steps_left, start_index, 10, num_threads);
    bool sequential11 = should_launch_task(37, total_steps, steps_left, start_index, 11, num_threads);
    bool sequential12 = should_launch_task(37, total_steps, steps_left, start_index, 12, num_threads);
    bool sequential13 = should_launch_task(37, total_steps, steps_left, start_index, 13, num_threads);
    bool sequential14 = should_launch_task(37, total_steps, steps_left, start_index, 14, num_threads);
    bool sequential15 = should_launch_task(37, total_steps, steps_left, start_index, 15, num_threads);
    bool sequential16 = should_launch_task(37, total_steps, steps_left, start_index, 16, num_threads);
    bool sequential17 = should_launch_task(37, total_steps, steps_left, start_index, 17, num_threads);
    bool sequential18 = should_launch_task(37, total_steps, steps_left, start_index, 18, num_threads);
    bool sequential19 = should_launch_task(37, total_steps, steps_left, start_index, 19, num_threads);
    bool sequential20 = should_launch_task(37, total_steps, steps_left, start_index, 20, num_threads);
    bool sequential21 = should_launch_task(37, total_steps, steps_left, start_index, 21, num_threads);
    bool sequential22 = should_launch_task(37, total_steps, steps_left, start_index, 22, num_threads);
    bool sequential23 = should_launch_task(37, total_steps, steps_left, start_index, 23, num_threads);
    bool sequential24 = should_launch_task(37, total_steps, steps_left, start_index, 24, num_threads);
    bool sequential25 = should_launch_task(37, total_steps, steps_left, start_index, 25, num_threads);
    bool sequential26 = should_launch_task(37, total_steps, steps_left, start_index, 26, num_threads);
    bool sequential27 = should_launch_task(37, total_steps, steps_left, start_index, 27, num_threads);
    bool sequential28 = should_launch_task(37, total_steps, steps_left, start_index, 28, num_threads);
    bool sequential29 = should_launch_task(37, total_steps, steps_left, start_index, 29, num_threads);
    bool sequential30 = should_launch_task(37, total_steps, steps_left, start_index, 30, num_threads);
    bool sequential31 = should_launch_task(37, total_steps, steps_left, start_index, 31, num_threads);
    bool sequential32 = should_launch_task(37, total_steps, steps_left, start_index, 32, num_threads);
    bool sequential33 = should_launch_task(37, total_steps, steps_left, start_index, 33, num_threads);
    bool sequential34 = should_launch_task(37, total_steps, steps_left, start_index, 34, num_threads);
    bool sequential35 = should_launch_task(37, total_steps, steps_left, start_index, 35, num_threads);
    bool sequential36 = should_launch_task(37, total_steps, steps_left, start_index, 36, num_threads);
    bool sequential37 = should_launch_task(37, total_steps, steps_left, start_index, 37, num_threads);
#else
    ...
    // M1 = (-(x) * A21 + x * x * A32 + 1.0 / (x) * A43) * (1.0 * B11 + 1.0 / (x) * B12 + 1.0 / (x) * B21 + -(x) * B31)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add1(A21, A32, A43, S1, x, sequential1);
#ifdef _PARALLEL_
    Matrix<Scalar> T1(mem_mngr.GetMem(start_index, 1, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T1(B11.m(), B11.n());
#endif
    T_Add1(B11, B12, B21, B31, T1, x, sequential1);
    FastMatmulRecursive(locker, mem_mngr, S1, T1, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S1.deallocate();
    T1.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 1, num_threads)) {
# pragma omp taskwait
...
# endif
    ...
    // M2 = (x * A21 + -(1.0 / (x)) * A43) * (1.0 * B11 + 1.0 / (x) * B21 + -(x) * B31)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add2(A21, A43, S2, x, sequential2);
#ifdef _PARALLEL_
    Matrix<Scalar> T2(mem_mngr.GetMem(start_index, 2, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T2(B11.m(), B11.n());
#endif
    T_Add2(B11, B21, B31, T2, x, sequential2);
    FastMatmulRecursive(locker, mem_mngr, S2, T2, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S2.deallocate();
    T2.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 2, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M3 = (-(1.0 / (x)) * A13 + 1.0 * A34) * (-(x) * B11 + -(x * x) * B31 + x * x * B41 + 1.0 * B42)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add3(A13, A34, S3, x, sequential3);
#ifdef _PARALLEL_
    Matrix<Scalar> T3(mem_mngr.GetMem(start_index, 3, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T3(B11.m(), B11.n());
#endif
    T_Add3(B11, B31, B41, B42, T3, x, sequential3);
    FastMatmulRecursive(locker, mem_mngr, S3, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S3.deallocate();
    T3.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 3, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M4 = (1.0 * A11 + -(1.0 / (x)) * A13 + 1.0 * A34) * (-1.0 * B11 + x * B41)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add4(A11, A13, A34, S4, x, sequential4);
#ifdef _PARALLEL_
    Matrix<Scalar> T4(mem_mngr.GetMem(start_index, 4, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T4(B11.m(), B11.n());
#endif
    T_Add4(B11, B41, T4, x, sequential4);
    FastMatmulRecursive(locker, mem_mngr, S4, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S4.deallocate();
    T4.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 4, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M5 = (1.0 * A12 + 1.0 * A24 + -(1.0 / (x)) * A44) * (-1.0 * B22 + x * x * B41 + x * B42)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add5(A12, A24, A44, S5, x, sequential5);
#ifdef _PARALLEL_
    Matrix<Scalar> T5(mem_mngr.GetMem(start_index, 5, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T5(B11.m(), B11.n());
#endif
    T_Add5(B22, B41, B42, T5, x, sequential5);
    FastMatmulRecursive(locker, mem_mngr, S5, T5, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S5.deallocate();
    T5.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 5, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M6 = (-1.0 * A12 + 1.0 / (x) * A44) * (x * B21 + -(x) * B41 + 1.0 / (x) * B42)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add6(A12, A44, S6, x, sequential6);
#ifdef _PARALLEL_
    Matrix<Scalar> T6(mem_mngr.GetMem(start_index, 6, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T6(B11.m(), B11.n());
#endif
    T_Add6(B21, B41, B42, T6, x, sequential6);
    FastMatmulRecursive(locker, mem_mngr, S6, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S6.deallocate();
    T6.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 6, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M7 = (1.0 * A11 + -(1.0 / (x)) * A13) * (1.0 * B11 + 1.0 / (x) * B12 + -(x) * B41)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add7(A11, A13, S7, x, sequential7);
#ifdef _PARALLEL_
    Matrix<Scalar> T7(mem_mngr.GetMem(start_index, 7, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T7(B11.m(), B11.n());
#endif
    T_Add7(B11, B12, B41, T7, x, sequential7);
    FastMatmulRecursive(locker, mem_mngr, S7, T7, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S7.deallocate();
    T7.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 7, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M8 = (-1.0 * A24 + x * x * A32 + 1.0 / (x) * A44) * (1.0 / (x) * B22 + -(x) * B41)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add8(A24, A32, A44, S8, x, sequential8);
#ifdef _PARALLEL_
    Matrix<Scalar> T8(mem_mngr.GetMem(start_index, 8, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T8(B11.m(), B11.n());
#endif
    T_Add8(B22, B41, T8, x, sequential8);
    FastMatmulRecursive(locker, mem_mngr, S8, T8, M8, total_steps, steps_left - 1, (start_index + 8 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S8.deallocate();
    T8.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 8, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M9 = (x * x * A32 + -(1.0 / (x)) * A43) * (1.0 * B11 + 1.0 / (x) * B22)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add9(A32, A43, S9, x, sequential9);
#ifdef _PARALLEL_
    Matrix<Scalar> T9(mem_mngr.GetMem(start_index, 9, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T9(B11.m(), B11.n());
#endif
    T_Add9(B11, B22, T9, x, sequential9);
    FastMatmulRecursive(locker, mem_mngr, S9, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S9.deallocate();
    T9.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 9, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M10 = (1.0 * A12 + 1.0 * A14 + x * A34 + -(1.0 / (x)) * A44) * (-(x) * B41 + 1.0 / (x) * B42)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
# pragma omp task if(sequential10) shared(mem_mngr, locker) untied default(shared)
...
#endif
    S_Add10(A12, A14, A34, A44, S10, x, sequential10);
#ifdef _PARALLEL_
    ...
    FastMatmulRecursive(locker, mem_mngr, S10, T10, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S10.deallocate();
    T10.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 10, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M11 = (x * A12 + x * x * A22 + -1.0 * A23 + 1.0 / (x) * A43) * (1.0 / (x) * B22 + x * B31 + -(x) * B32)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add11(A12, A22, A23, A43, S11, x, sequential11);
#ifdef _PARALLEL_
    Matrix<Scalar> T11(mem_mngr.GetMem(start_index, 11, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T11(B11.m(), B11.n());
#endif
    T_Add11(B22, B31, B32, T11, x, sequential11);
    FastMatmulRecursive(locker, mem_mngr, S11, T11, M11, total_steps, steps_left - 1, (start_index + 11 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S11.deallocate();
    T11.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 11, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M12 = (1.0 / (x) * A43) * (1.0 * B11 + 1.0 / (x) * B12 + 1.0 / (x) * B22 + 1.0 * B32)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
# pragma omp task if(sequential12) shared(mem_mngr, locker) untied default(shared)
    ...
    FastMatmulRecursive(locker, mem_mngr, A43, T12, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T12.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 12, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M13 = (1.0 * A11 + -(1.0 / (x)) * A13 + x * x * A32 + 1.0 * A33 + -1.0 * A41) * (-(1.0 / (x)) * B12)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add13(A11, A13, A32, A33, A41, S13, x, sequential13);
    M13.UpdateMultiplier(Scalar(-(1.0 / (x))));
    FastMatmulRecursive(locker, mem_mngr, S13, B12, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S13.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 13, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M14 = (-(x) * A21 + x * x * A31 + x * A34 + 1.0 * A41 + 1.0 / (x) * A43) * (1.0 / (x) * B11)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add14(A21, A31, A34, A41, A43, S14, x, sequential14);
    M14.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, S14, B11, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S14.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 14, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M15 = (-(x) * A21 + x * x * A22 + x * A42 + 1.0 / (x) * A43 + -(1.0 / (x)) * A44) * (1.0 / (x) * B21)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add15(A21, A22, A42, A43, A44, S15, x, sequential15);
    M15.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, S15, B21, M15, total_steps, steps_left - 1, (start_index + 15 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S15.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 15, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M16 = (-(1.0 / (x)) * A13 + x * x * A31 + x * A33 + -(1.0 / (x)) * A43) * (1.0 / (x) * B12 + 1.0 * B32)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add16(A13, A31, A33, A43, S16, x, sequential16);
#ifdef _PARALLEL_
    Matrix<Scalar> T16(mem_mngr.GetMem(start_index, 16, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T16(B11.m(), B11.n());
#endif
    T_Add16(B12, B32, T16, x, sequential16);
    FastMatmulRecursive(locker, mem_mngr, S16, T16, M16, total_steps, steps_left - 1, (start_index + 16 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S16.deallocate();
    T16.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 16, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M17 = (1.0 / (x) * A13) * (-(x * x) * B11 + 1.0 / (x) * B12 + 1.0 * B32 + x * B42)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add17(B11, B12, B32, B42, T17, x, sequential17);
    M17.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, A13, T17, M17, total_steps, steps_left - 1, (start_index + 17 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T17.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 17, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M18 = (1.0 * A23 + -(1.0 / (x)) * A43) * (1.0 / (x) * B22 + x * B31)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add18(A23, A43, S18, x, sequential18);
#ifdef _PARALLEL_
    Matrix<Scalar> T18(mem_mngr.GetMem(start_index, 18, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T18(B11.m(), B11.n());
#endif
    T_Add18(B22, B31, T18, x, sequential18);
    FastMatmulRecursive(locker, mem_mngr, S18, T18, M18, total_steps, steps_left - 1, (start_index + 18 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S18.deallocate();
    T18.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 18, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M19 = (-(x) * A21 + 1.0 * A41 + 1.0 / (x) * A43) * (1.0 / (x) * B11 + 1.0 / (x) * B12)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
# pragma omp task if(sequential19) shared(mem_mngr, locker) untied default(shared)
...
#endif
    S_Add19(A21, A41, A43, S19, x, sequential19);
#ifdef _PARALLEL_
    ...
    FastMatmulRecursive(locker, mem_mngr, S19, T19, M19, total_steps, steps_left - 1, (start_index + 19 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S19.deallocate();
    T19.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 19, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
#endif
    T_Add20(B12, B31, T20, x, sequential20);
    M20.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, A33, T20, M20, total_steps, steps_left - 1, (start_index + 20 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T20.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 20, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
#endif
    T_Add21(B21, B41, T21, x, sequential21);
    M21.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, A44, T21, M21, total_steps, steps_left - 1, (start_index + 21 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T21.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 21, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M22 = (-1.0 * A23 + 1.0 * A24 + 1.0 * A42 + -(1.0 / (x)) * A44) * (1.0 / (x) * B22)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add22(A23, A24, A42, A44, S22, x, sequential22);
    M22.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, S22, B22, M22, total_steps, steps_left - 1, (start_index + 22 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S22.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 22, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
#endif
    T_Add23(B21, B22, T23, x, sequential23);
    M23.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, A42, T23, M23, total_steps, steps_left - 1, (start_index + 23 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T23.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 23, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M24 = (1.0 / (x) * A13 + 1.0 / (x) * A14) * (1.0 * B42)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add24(A13, A14, S24, x, sequential24);
    M24.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, S24, B42, M24, total_steps, steps_left - 1, (start_index + 24 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S24.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 24, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M25 = (-1.0 * A25 + -(x) * A52 + 1.0 * A53 + x * x * A55) * (1.0 / (x) * B52)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add25(A25, A52, A53, A55, S25, x, sequential25);
    M25.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, S25, B52, M25, total_steps, steps_left - 1, (start_index + 25 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S25.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 25, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M26 = (1.0 * A15 + -(x) * A35 + 1.0 * A51 + x * x * A55) * (x * B11 + 1.0 / (x) * B51 + -(1.0 / (x)) * B52)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add26(A15, A35, A51, A55, S26, x, sequential26);
#ifdef _PARALLEL_
    Matrix<Scalar> T26(mem_mngr.GetMem(start_index, 26, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T26(B11.m(), B11.n());
#endif
    T_Add26(B11, B51, B52, T26, x, sequential26);
    FastMatmulRecursive(locker, mem_mngr, S26, T26, M26, total_steps, steps_left - 1, (start_index + 26 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S26.deallocate();
    T26.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 26, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M27 = (-(x) * A35 + 1.0 * A54) * (x * B41 + -(1.0 / (x)) * B51)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add27(A35, A54, S27, x, sequential27);
#ifdef _PARALLEL_
    Matrix<Scalar> T27(mem_mngr.GetMem(start_index, 27, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T27(B11.m(), B11.n());
#endif
    T_Add27(B41, B51, T27, x, sequential27);
    FastMatmulRecursive(locker, mem_mngr, S27, T27, M27, total_steps, steps_left - 1, (start_index + 27 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S27.deallocate();
    T27.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 27, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M28 = (1.0 * A15 + x * A52) * (1.0 * B21 + 1.0 / (x) * B52)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add28(A15, A52, S28, x, sequential28);
#ifdef _PARALLEL_
    Matrix<Scalar> T28(mem_mngr.GetMem(start_index, 28, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T28(B11.m(), B11.n());
#endif
    T_Add28(B21, B52, T28, x, sequential28);
    FastMatmulRecursive(locker, mem_mngr, S28, T28, M28, total_steps, steps_left - 1, (start_index + 28 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S28.deallocate();
    T28.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 28, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M29 = (1.0 * A25 + 1.0 * A51) * (-(1.0 / (x)) * B51 + 1.0 / (x) * B52)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add29(A25, A51, S29, x, sequential29);
#ifdef _PARALLEL_
    Matrix<Scalar> T29(mem_mngr.GetMem(start_index, 29, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T29(B11.m(), B11.n());
#endif
    T_Add29(B51, B52, T29, x, sequential29);
    FastMatmulRecursive(locker, mem_mngr, S29, T29, M29, total_steps, steps_left - 1, (start_index + 29 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S29.deallocate();
    T29.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 29, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M30 = (x * A35 + 1.0 * A53) * (x * B31 + -(1.0 / (x)) * B52)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add30(A35, A53, S30, x, sequential30);
#ifdef _PARALLEL_
    Matrix<Scalar> T30(mem_mngr.GetMem(start_index, 30, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T30(B11.m(), B11.n());
#endif
    T_Add30(B31, B52, T30, x, sequential30);
    FastMatmulRecursive(locker, mem_mngr, S30, T30, M30, total_steps, steps_left - 1, (start_index + 30 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S30.deallocate();
    T30.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 30, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M31 = (1.0 * A45 + -(x) * A52) * (1.0 * B22 + 1.0 / (x) * B52)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add31(A45, A52, S31, x, sequential31);
#ifdef _PARALLEL_
    Matrix<Scalar> T31(mem_mngr.GetMem(start_index, 31, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T31(B11.m(), B11.n());
#endif
    T_Add31(B22, B52, T31, x, sequential31);
    FastMatmulRecursive(locker, mem_mngr, S31, T31, M31, total_steps, steps_left - 1, (start_index + 31 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S31.deallocate();
    T31.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 31, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
#endif
    T_Add32(B42, B51, T32, x, sequential32);
    M32.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A54, T32, M32, total_steps, steps_left - 1, (start_index + 32 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T32.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 32, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
#endif
    S_Add33(A25, A54, S33, x, sequential33);
    M33.UpdateMultiplier(Scalar(1.0 / (x)));
    FastMatmulRecursive(locker, mem_mngr, S33, B51, M33, total_steps, steps_left - 1, (start_index + 33 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S33.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 33, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M34 = (1.0 * A15) * (x * B11 + 1.0 * B21 + 1.0 / (x) * B51)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add34(B11, B21, B51, T34, x, sequential34);
    M34.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A15, T34, M34, total_steps, steps_left - 1, (start_index + 34 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T34.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 34, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
...
#endif
    T_Add35(B32, B52, T35, x, sequential35);
    M35.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A53, T35, M35, total_steps, steps_left - 1, (start_index + 35 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T35.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 35, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M36 = (-1.0 * A45 + 1.0 * A51) * (x * B12 + 1.0 / (x) * B51 + -(1.0 / (x)) * B52)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add36(A45, A51, S36, x, sequential36);
#ifdef _PARALLEL_
    Matrix<Scalar> T36(mem_mngr.GetMem(start_index, 36, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
#else
    Matrix<Scalar> T36(B11.m(), B11.n());
#endif
    T_Add36(B12, B51, B52, T36, x, sequential36);
    FastMatmulRecursive(locker, mem_mngr, S36, T36, M36, total_steps, steps_left - 1, (start_index + 36 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S36.deallocate();
    T36.deallocate();
#endif
...
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 36, num_threads)) {
# pragma omp taskwait
# if defined(_PARALLEL_) && (_PARALLEL_ == _HYBRID_PAR_)
    SwitchToDFS(locker, num_threads);
# endif
    ...
    // M37 = (1.0 * A45) * (x * B12 + 1.0 * B22 + 1.0 / (x) * B51)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add37(B12, B22, B51, T37, x, sequential37);
    M37.UpdateMultiplier(Scalar(1.0));
    FastMatmulRecursive(locker, mem_mngr, A45, T37, M37, total_steps, steps_left - 1, (start_index + 37 - 1) * 37, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T37.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
    ...
    if (should_task_wait(37, total_steps, steps_left, start_index, 37, num_threads)) {
# pragma omp taskwait
    }
#endif

    M_Add1(M3, M4, M6, M10, M21, M24, M34, C11, x, false, beta);
    M_Add2(M3, M4, M5, M7, M8, M10, M17, M24, M28, C12, x, false, beta);
    M_Add3(M2, M8, M9, M15, M18, M21, M22, M23, M32, M33, C21, x, false, beta);
    M_Add4(M1, M2, M5, M6, M8, M9, M11, M12, M18, M25, M35, C22, x, false, beta);
    M_Add5(M1, M2, M4, M7, M13, M14, M19, M20, M27, M32, C31, x, false, beta);
    M_Add6(M3, M9, M12, M16, M17, M20, M30, M35, C32, x, false, beta);
    M_Add7(M2, M14, M15, M21, M37, C41, x, false, beta);
    M_Add8(M1, M2, M4, M6, M7, M9, M12, M13, M20, M23, M31, C42, x, false, beta);
    M_Add9(M25, M26, M27, M28, M29, M30, M33, M34, C51, x, false, beta);
    M_Add10(M25, M29, M31, M32, M33, M35, M36, M37, C52, x, false, beta);
...
#endif
    DynamicPeeling(A, B, C, 5, 5, 2, beta);
}
...
#ifdef _PARALLEL_
    mem_mngr.Allocate(5, 5, 2, 37, num_steps, A.m(), A.n(), B.n());
#endif
    A.set_multiplier(alpha);
    int num_multiplies_per_step = 37;
...
#endif  // _smirnov552_37_262_approx_HPP_
